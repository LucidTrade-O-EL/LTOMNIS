...
const ActionItem: React.FC<{ text: string, isCompleted: boolean, status: string }> = ({ text, isCompleted, status }) => {
  ...
  return (
    <View style={{ flexDirection: 'column', width: '100%', alignItems: 'center' }}>
      ...
      <View style={{ flexDirection: 'column', width: '100%', alignItems: 'center' }}>
        <Divider width={1} color={GlobalStyles.Colors.accent250} style={{ marginVertical: 5, width: '100%', alignItems: 'center' }} />
      </View>
    </View>
  );
};
...
const ActionItemList: React.FC = () => {
    ...
    return (
      <FlatList
        ...
        renderItem={({ item }) => <ActionItem text={item.text} isCompleted={item.isCompleted} status={item.status} />}
      />
    );
  };
